/**
 Test Name: TC_HR_Transfer_of_an_Employee
 Created By: Ciber
 Creation Date:April 20,2018
 Modified By:
 Modified Date:
 Test Description: This script is used for to HR Transfer of an Employee
...
public class TC_HR_Transfer_of_an_Employee extends BaseWebTest  {
SoftAssert sAssert;

public WebDriverWait wdwait;

@Test(dataProvider = "CloudDataProviderUtil_HCM", dataProviderClass = DataProviderRepository.class, description = "This test is to HR Transfer of an Employee")	
public void HR_Transfer_of_an_Employee(String UserName,String Password,String SearchName,String FullName,String ActionType,String ActionReason,String StartDate,String LocationSearch,String LocationName) throws TestException,InterruptedException, TestException
{
	...
	report.log(LogStatus.INFO, "Step 13. Click the Action Reason list. and Click the Career Progression list item.");
	objPMP.selectActionReason(ActionReason);
	...
	report.log(LogStatus.INFO, "Step 26. Verify that ActionType"+ActionType+" is updated with expected one or not");
	sAssert.assertEquals(objPMP.getActionType(),ActionType,"Actaul and Exepced ActionType is not matching ");
	
	report.log(LogStatus.INFO, "Step 27. Verify ActionReason"+ActionReason+" is updated with expected one or not");
	sAssert.assertEquals(objPMP.getActionReason(),ActionReason,"Actaul and Exepced ActionType is not matching ");
	
	report.log(LogStatus.INFO, "Step 28. Click the Submit button.");
	objPMP.clickLocationSubmitButton();
	
	report.log(LogStatus.INFO, "Step 29. Click the Yes button on confirmation dailog");
	objPMP.clickConfirmatioDialog("Yes");
	
	report.log(LogStatus.INFO, "Step 30. Click the OK button.");
	objPMP.clickOKButton();
	
	report.log(LogStatus.INFO, "Step 31. Click the Done button");
