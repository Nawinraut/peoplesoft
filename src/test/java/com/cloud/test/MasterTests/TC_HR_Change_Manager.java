/**
 Test Name: TC_HR_Change_Manager
 Created By: Ciber
 Creation Date:April 20,2018
 Modified By:
 Modified Date:
 Test Description: This script is used for to HR Change Manager
...
public class TC_HR_Change_Manager extends BaseWebTest  {
SoftAssert sAssert;


public WebDriverWait wdwait;

@Test(dataProvider = "CloudDataProviderUtil_HCM", dataProviderClass = DataProviderRepository.class, description = "This test is to HR Change Manager")	
public void HR_Change_Manager(String UserName,String Password,String SearchName,String FullName,String ActionType,String StartDate,String ManagerSearch,String ManagerName) throws TestException,InterruptedException, TestException
{
	...
	//objPMP.selectPersonName("Armstrong, Elliot");
	...
	report.log(LogStatus.INFO, "Step 17. Click the Add Manager Details button");
	objPMP.clickAddManagerDetailsButton();
	
	report.log(LogStatus.INFO, "Step 18. Click the Name list");
	objPMP.clickNameList();
	...
	objPMP.enterLocationName(ManagerSearch);
	
	report.log(LogStatus.INFO, "Step 21. Click the Search button.");
	objPMP.clickSearchButton();
	
	report.log(LogStatus.INFO, "Step 22. Click the Name cell");
	objPMP.selectManagerName(ManagerName);
	
	
	report.log(LogStatus.INFO, "Step 23. Click the OK button.");
	objPMP.clickBUOKButton();
	
	report.log(LogStatus.INFO, "Step 24. Click the Remove button.");
	objPMP.removeManager();
	
	report.log(LogStatus.INFO, "Step 25. Click the Type list.");
	report.log(LogStatus.INFO, "Step 26. Click the Line manager list item.");
	objPMP.selectManagerType("Line manager");
	
	report.log(LogStatus.INFO, "Step 27. Scroll up to click the Review button");
	SeleniumUtils.scroll(driver, 0, -1500);
	
	report.log(LogStatus.INFO, "Step 28. Click the Review button.");
	objPMP.clickLocationReviewButton();
	
	report.log(LogStatus.INFO, "Step 29. Click the Submit button.");
	objPMP.clickReviewSubmitButton();
	
	report.log(LogStatus.INFO, "Step 30. Click the Yes button on confirmation dailog");
	objPMP.clickConfirmatioDialog("Yes");
	
	report.log(LogStatus.INFO, "Step 31. Click the OK button.");
	objPMP.clickOKButton();
	
	report.log(LogStatus.INFO, "Step 32. Click the Done button");
	objPMP.clickDoneButton();
	
	/*report.log(LogStatus.INFO, "Step 33. Click the Yes button");
