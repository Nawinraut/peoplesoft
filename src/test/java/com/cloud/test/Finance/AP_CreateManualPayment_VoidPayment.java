...
public class AP_CreateManualPayment_VoidPayment extends BaseWebTest{
	
	SoftAssert sAssert;
	public WebDriverWait wdwait;
	@Test(dataProvider = "CloudDataProviderUtil_Finance", dataProviderClass =DataProviderRepository.class, description = "This test is How to Create Manual Payment and Void the Same Payment by Payables Supervisor")
	public void AP_Create_Manual_Payment(String UserName,String Password,String BusinessUnit,String Supplier,String SupplierCell,String DisbursementBankAccount,String PaymentProcessProfile,String InvoiceNumber,String ReIssueDate)throws TestException,InterruptedException, TestException {
		...
		CreatePaymentPage objCreatePayment=objOverview.clickCreatePaymentLink();
			...
			report.log(LogStatus.INFO, "Step. Void the Payament Starts from this Step");
			...
			report.log(LogStatus.INFO, "Step 2. Click on Payments link");
			OverviewPage objOverview1=objHome.clickPayments();
			
			report.log(LogStatus.INFO, "Step 3. Click on Taksks button ");
			sAssert.assertTrue(objOverview.isTasksBtnDisplayed());
			objOverview.clickTasksButton();
		
			report.log(LogStatus.INFO, "Step 4. Click on Manage Payments link");
			...
			report.log(LogStatus.INFO, "Step 5. Click on Party Search Icon");
			objMngPayment.isPartySearchIconDisplayed();
			objMngPayment.clickPartySearchIcon();
			report.log(LogStatus.INFO, "Step 5.1 Click on Party Search link");
			objMngPayment.isPartySearchLinkDisplayed();
			objMngPayment.clickPartySearchLink();	
			report.log(LogStatus.INFO, "Step 5.2 Enter the Party Value in Suplier Or Party text field");
			objMngPayment.isPartyValueDisplayed();
			objMngPayment.enterPartyValue(Supplier);
			objMngPayment.clickSupplierNumber();
			report.log(LogStatus.INFO, "Step 5.3 Click on Party Search button");
			objMngPayment.isPartySearchButtonDisplayed();
			objMngPayment.clickPartySearchButton();
			report.log(LogStatus.INFO, "Step 5.4 Select Party from dropdown");
			objMngPayment.selectPartyName(Supplier);
			report.log(LogStatus.INFO, "Step 5.5 Click on Party Ok button");
			objMngPayment.isPartyOkButtonDisplayed();
			objMngPayment.clickPartyOkButton();
			
			report.log(LogStatus.INFO, "Step 6. Enter the Payment Number in Payment Number text field");
			objMngPayment.isPaymentNumberDisplayed();
			objMngPayment.enterPaymentNumber(paymentNumber);
			
			report.log(LogStatus.INFO, "Step 7. Click on Search button");
			objMngPayment.isSearchBtnDisplayed();
			objMngPayment.clickSearchButton();
			
			report.log(LogStatus.INFO, "Step 8. Select Payment Number link");
			...
			report.log(LogStatus.INFO, "Step 9. Click on Actions Manu");
			objPayment.clickActionsMenu();
			
			report.log(LogStatus.INFO, "Step 10. Select Void from Actions Manu");
			objPayment.isVoidListDisplayed();
			objPayment.selectVoidList();
			
			report.log(LogStatus.INFO, "Step 11. Click on Submit button");
			objPayment.isSubmitBtnDisplayed();
			objPayment.clickSubmitButton();
			
			report.log(LogStatus.INFO, "Step 12. Click on Done button");
			objPayment.isDoneBtnDisplayed();
			objPayment.clickDoneButton();
			
			report.log(LogStatus.INFO, "Step 13. Click on Done button");
