...
public class AP_CreateManualPayment_ReissuePayment extends BaseWebTest{

	
	SoftAssert sAssert;
	public WebDriverWait wdwait;
	@Test(dataProvider = "CloudDataProviderUtil_Finance", dataProviderClass =DataProviderRepository.class, description = "This test is How to Create Manual Payment and Reissue the same Payment by Payables Supervisor")
	public void AP_Create_Manual_Payment(String UserName,String Password,String BusinessUnit,String Supplier,String SupplierCell,String DisbursementBankAccount,String PaymentProcessProfile,String InvoiceNumber,String ReIssueDate)throws TestException,InterruptedException, TestException {
		...
		CreatePaymentPage objCreatePayment=objOverview.clickCreatePaymentLink();
			...
			String paymentNumber = objCreatePayment.fetchPaymentNumber();
				
			report.log(LogStatus.INFO, "Step 19. Click on Confirmation Ok button");
			objCreatePayment.clickConfOkButton();
			
report.log(LogStatus.INFO, "Step. Reissue the Payament Starts from this step");
			
			report.log(LogStatus.INFO, "Step 1. Click on Home image");
			objHome.clickImgHome();
			
			report.log(LogStatus.INFO, "Step 2. Click on Navigator Icon");
			objHome.clickNavigatorIcon();
			
			report.log(LogStatus.INFO, "Step 3. Click on Payments link");
			OverviewPage objOverview2=objHome.clickPayments();
			
			report.log(LogStatus.INFO, "Step 4. Click on Taksks button ");
			sAssert.assertTrue(objOverview.isTasksBtnDisplayed());
			objOverview.clickTasksButton();
			
			report.log(LogStatus.INFO, "Step 5. Click on Manage Payments link");
			...
			report.log(LogStatus.INFO, "Step 6. Click on Party Search Icon");
			objMngPayment.isPartySearchIconDisplayed();
			objMngPayment.clickPartySearchIcon();
			report.log(LogStatus.INFO, "Step 6.1 Click on Party Search link");
			objMngPayment.isPartySearchLinkDisplayed();
			objMngPayment.clickPartySearchLink();	
			report.log(LogStatus.INFO, "Step 6.2 Enter the Party Value in Suplier Or Party text field");
			objMngPayment.isPartyValueDisplayed();
			objMngPayment.enterPartyValue(Supplier);
			objMngPayment.clickSupplierNumber();
			report.log(LogStatus.INFO, "Step 6.3 Click on Party Search button");
			objMngPayment.isPartySearchButtonDisplayed();
			objMngPayment.clickPartySearchButton();
			report.log(LogStatus.INFO, "Step 6.4 Select Party from dropdown");
			objMngPayment.selectPartyName(Supplier);
			report.log(LogStatus.INFO, "Step 6.5 Click on Party Ok button");
			objMngPayment.isPartyOkButtonDisplayed();
			objMngPayment.clickPartyOkButton();
			
			report.log(LogStatus.INFO, "Step 7. Enter the Payment Number in Payment Number text field");
			...
			report.log(LogStatus.INFO, "Step 9. Select Payment Number link");
			//objMngPayment.selectPaymentNumber(PaymentNumber);
			
			//PaymentPage objPayment = new PaymentPage(driver, report);
			PaymentPage objPayment1=objMngPayment.selectPaymentNumberCell(paymentNumber);
			objPayment1.iskActionsMenuDisplayed();
			report.log(LogStatus.INFO, "Step 10. Click on Actions Manu");
			objPayment1.clickActionsMenu();
			
			report.log(LogStatus.INFO, "Step 11. Select Reissue list from Actions Manu");
			objPayment1.isReissueListDisplayed();
			objPayment1.selectReissueList();
			
			if (ReIssueDate!=""){
				String reIssueDateParts[] = ReIssueDate.split("-");
				String reIssueMonth  = reIssueDateParts[0];
				String reIssueDay  = reIssueDateParts[1];
				String reIssueYear = reIssueDateParts[2];
				report.log(LogStatus.INFO, "Step 11. Click on ReIssue Date button");
				objPayment1.clickReissueDateButton();
				/*report.log(LogStatus.INFO, "Step 11. Enter on ReIssue Date Year");
				objPayment1.SelectYearFromDatePickerReissueDate(reIssueYear);*/
				report.log(LogStatus.INFO, "Step 11.1. Select From ReIssue Month");
				objPayment1.selectReissueDateMonth(reIssueMonth);
				report.log(LogStatus.INFO, "Step 11.2. Select From ReIssue Date");
				objPayment1.selectReissueDate(reIssueDay);
			}
			
			report.log(LogStatus.INFO, "Step 12. Click on Submit button");
			objPayment1.isReissueSubmitBtnDisplayed();
			objPayment1.clickReissueSubmitButton();
			
			report.log(LogStatus.INFO, "Step 13. Click on Conformation Ok button");
			objPayment1.isConfOkButtonDisplayed();
			objPayment1.clickConfOkButton();
			
			report.log(LogStatus.INFO, "Step 14. Click on Done button");
			objPayment1.isDoneBtnDisplayed();
			objPayment1.clickDoneButton();
