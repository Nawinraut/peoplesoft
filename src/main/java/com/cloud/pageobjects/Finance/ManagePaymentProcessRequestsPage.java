...
public class ManagePaymentProcessRequestsPage extends BasePageObject{
    ...
    private WebElement btnSearch;
 	...
 	@FindBy(how=How.XPATH, using = "//img[contains(@id,'commandImageLink2_2::icon')]|//img[contains(@id,'commandImageLink2_1::icon')]")
    private WebElement imgAction; 
 	
 	@FindBy(how=How.XPATH, using = "//*[contains(@id,'_FOd1::msgDlg::cancel')]")
    private WebElement btnConfOk;
 	
 	@FindBy(how=How.XPATH, using = "//*[contains(@id,'pt1:AP1:SPb')]")
    private WebElement btnDone;
 	
 	@FindBy(how=How.XPATH, using = "//img[contains(@id,'pt1:AP1:ATT2:_ATTp:tti2::icon')]")
	...
	// Action Image
	   ...
	   /* public ReviewInstallmentsPage clickActionImage()throws TestException{
	    ...
	    public PrintPaymentDocumentsPage clickActionImage()throws TestException{
	    	imgAction.click();
	    	report.log(LogStatus.PASS, "Clicked on Action Image");
	    	return new PrintPaymentDocumentsPage(driver,report);
	    }
	    
	    public PaymentFilePage clickActionImg()throws TestException{
	    	imgAction.click();
	    	report.log(LogStatus.PASS, "Clicked on Action Image");
	    	return new PaymentFilePage(driver,report);
	    }
	    
	    public String fetchPaymentFileNumber(){
	    	btnRefresh.click();
	    	SeleniumUtils.delay(5000);
	    	btnRefresh.click();
	    	SeleniumUtils.delay(5000);
		   JavascriptExecutor js = (JavascriptExecutor) driver; 
	    	List<WebElement> InvoiceRows = driver.findElements(By.xpath("//div[contains(@id,'pt1:AP1:ATT2:_ATTp:tt1::db')]//div//td[1]"));
	    	System.out.println("Statement Rows are"+InvoiceRows.size());
	    	
	    	if(InvoiceRows.size()==2) {
	    		String PaymentFileData = driver.findElement(By.xpath("//div[contains(@id,'pt1:AP1:ATT2:_ATTp:tt1::db')]//tbody//tr[2]//div//td[1]")).getText();
				System.out.println("Data is"+PaymentFileData);
				if(PaymentFileData!="") {
					 
					 this.getScreenDetails();
					 report.log(LogStatus.PASS, "Payment Number is "+PaymentFileData);
					 return PaymentFileData;
				 }else {
			    		report.log(LogStatus.INFO, "Payment File Data is not available");
				 }	
				 }
			return null;			   
		  }
